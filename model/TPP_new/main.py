...
from .TPP_both import TPP
from utils.EarlyStopping import EarlyStopping
torch.set_num_threads(1)

log = get_logger(__name__)

pre_views = ['TPP_preview']
post_views = ['TPP_post_view']  

model = None

def train_model(model: TPP, train_dataloader: DataLoader, valid_dataloader: DataLoader, metric_keys, args):
...
def train(dataloader: dict[DataLoader], dataset_name, **kw):
    global model
    with open('./model/TPP/TPP.yaml', 'r', encoding='utf-8') as f:
...
def inference(dataloader: DataLoader, **kv):
    ...
    with torch.no_grad():
       ...
       for batch_data in tqdm(dataloader):
            batch_data_device = {k: v.to(model.device, non_blocking=True) for k, v in batch_data.items() if 'y_POI_id' not in k and 'POI_catname' not in k}
            batch_data_device['y_POI_id'] = {k: v.to(model.device, non_blocking=True) for k, v in batch_data['y_POI_id'].items() if 'POI_catname' not in k}
            ...
            print(y_predict_time.detach().cpu().numpy().reshape(-1).tolist())
            print(batch_data['y_POI_id']['time_delta'].detach().cpu().numpy().tolist())
