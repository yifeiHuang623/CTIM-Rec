...
from .TPP_utils import TimePositionalEncoding, ScaledSoftplus, EventSampler, \
    FourierPeriodicEncoder
...
class TPP(nn.Module):
    ...
    def handle_sequence(self, batch_data):
        poi_id, time, time_delta, poi_type = batch_data['POI_id'], batch_data['timestamps'], batch_data['time_delta'], batch_data['POI_catid']
        user_id = batch_data['user_id'].unsqueeze(dim=1).expand(poi_id.shape[0], poi_id.shape[1])
        
        seq_len = batch_data['mask']
        
        y_poi_id, y_time, y_time_delta, y_type = batch_data['y_POI_id']['POI_id'], batch_data['y_POI_id']['timestamps'], batch_data['y_POI_id']['time_delta'], \
            batch_data['y_POI_id']['POI_catid']
        ...
        y_type_seq = torch.full((B, L), 0, dtype=torch.float, device=self.device)
        ...
        for i in range(B):
            ...
            y_type_seq[i, :end] = torch.cat((poi_type[i, 1:end], y_type[i].unsqueeze(dim=-1)), dim=-1)
        ...
        batch_data['y_POI_id']['POI_catid'] = y_type_seq
    ...
    def calculate_both_loss(self, batch_data):
        ...
        indices = torch.arange(y_poi_seqs.shape[1]).unsqueeze(0).expand(y_poi_seqs.shape[0], y_poi_seqs.shape[1]).to(self.device)
        y_invalid_positions = (indices >= seq_len.unsqueeze(1)).to(torch.bool)
        # attention_mask = attention_mask | invalid_positions.unsqueeze(1).expand(-1, poi_seqs.shape[1], -1)
        
        user_emb = self.user_embed_model(u_id)
        
        time_norm_seqs = (time_seqs - time_seqs[:, 0:1]) / 3600
        enc_out = self.forward(time_seqs=time_norm_seqs, poi_seqs=poi_seqs, attention_mask=attention_mask, poi_cat_seq=poi_cat_seqs, seq_mask=invalid_positions)
        ...
        # time_distance_factor = self.alpha * distance_decay + self.beta * time_delta_decay
    ...
    def predict(self, batch_data):
        ...
        u_id, poi_seqs, poi_cat_seqs, time_seqs, time_delta, seq_len = batch_data['user_id'], batch_data['POI_id'], batch_data['POI_catid'], batch_data['timestamps'], \
                                                                     batch_data['time_delta'], batch_data['mask']
        
        y_time_delta, y_POI, y_POI_type = batch_data['y_POI_id']['time_delta'], batch_data['y_POI_id']['POI_id'], batch_data['y_POI_id']['POI_catid']
        ...
        # [batch_size, seq_len, num_sample]
        # accepted_dtimes, weights, lambda_t, pdf, lambda_t_poi = self.event_sampler.draw_next_time_one_step(time_seqs=time_norm_seqs,
        #                                                                     absolute_time_seqs=time_seqs,
        #                                                                     time_delta_seqs=time_delta,
        #                                                                     poi_seqs=poi_seqs,
        #                                                                     user_seqs=u_id,
        #                                                                     poi_cat_seqs=poi_cat_seqs,
        #                                                                     attention_mask=attention_mask,
        #                                                                     seq_mask=invalid_positions,
        #                                                                     intensity_fn=self.compute_intensities_at_sample_times,
        ...
        intensities_at_times, user_int, distance_int, period_int, history_int, base_int = self.compute_intensities_at_sample_times(time_seqs=time_norm_seqs,
                                                                        absolute_time_seqs=time_seqs,
                                                                        poi_seqs=poi_seqs,
                                                                        user_seq=u_id,
        ...
        intensities_weighted_ret = torch.einsum('...s,...sm->...m', weights, intensities_normalized)
        dtimes_pred_ret = torch.sum(accepted_dtimes * weights, dim=-1)
        '''
        batch_indices = torch.arange(batch_data['mask'].shape[0])
        intensities_weighted = intensities_weighted_ret[batch_indices, batch_data['mask'] - 1] 
        y_POI = y_POI[batch_indices, batch_data['mask'] - 1]
        poi_seqs = poi_seqs[batch_indices, batch_data['mask'] - 1]
        user_int = user_int[batch_indices, batch_data['mask'] - 1]
        distance_int = distance_int[batch_indices, batch_data['mask'] - 1]
        period_int = period_int[batch_indices, batch_data['mask'] - 1]
        history_int = history_int[batch_indices, batch_data['mask'] - 1]
        y_time_delta = y_time_delta[batch_indices, batch_data['mask'] - 1]
        y_POI_type = y_POI_type[batch_indices, batch_data['mask'] - 1]
        
        pred_poi_indices = torch.argmax(intensities_weighted, dim=-1)
        final_mask = (pred_poi_indices == y_POI)
        
        history_seq = batch_data['POI_id'][final_mask].cpu().numpy().tolist()
        timestamp = time_seqs[batch_indices, batch_data['mask'] - 1][final_mask].detach().cpu().numpy().tolist()
        sel_true_time = y_time_delta[final_mask].cpu().numpy().tolist()
        distance_matrix = self.distance_matrix[poi_seqs[final_mask].cpu().numpy()].tolist()
        
        user_int = user_int[final_mask].squeeze().detach().cpu().numpy().tolist()
        distance_int = distance_int[final_mask].squeeze().detach().cpu().numpy().tolist()
        period_int = period_int[final_mask].squeeze().detach().cpu().numpy().tolist()
        history_int = history_int[final_mask].squeeze().detach().cpu().numpy().tolist()
        sel_true_type = y_POI_type[final_mask].detach().cpu().numpy().tolist()
        base_int = base_int.squeeze().detach().cpu().numpy().tolist()
        base_int = [base_int.copy() for _ in range(len(user_int))]
        
        data = {
            'user_id': u_id[final_mask].cpu().numpy().tolist(),
            'true_POI': y_POI[final_mask].cpu().numpy().tolist(),
            'true_POI_type': sel_true_type,
            'history': history_seq,
            'based_timestamp': timestamp,
            'true_time_delat': sel_true_time,
            'distance': distance_matrix,
            'user_int': user_int,
            'distance_int': distance_int,
            'period_int': period_int,
            'history_int': history_int,
            'base_int': base_int
        }
        
        resulting_df = pd.DataFrame(data)
        
        path = '/home/huangyf/TGN/Traj_TPP4NPP/analysis/case_study.csv'
        if os.path.exists(path):
            resulting_df.to_csv(path, mode='a', header=None, index=False)
        else:
            resulting_df.to_csv(path, mode='a', header=True, index=False)
        '''
        '''
        batch_indices = torch.arange(batch_data['mask'].shape[0])
        intensities_weighted = intensities_weighted_ret[batch_indices, batch_data['mask'] - 1] 
        dtimes_pred = dtimes_pred_ret[batch_indices, batch_data['mask'] - 1]
        y_POI = y_POI[batch_indices, batch_data['mask'] - 1]
        y_time_delta = y_time_delta[batch_indices, batch_data['mask'] - 1]
        y_POI_type = y_POI_type[batch_indices, batch_data['mask'] - 1]
        lambda_t = lambda_t[batch_indices, batch_data['mask'] - 1]
        pdf = pdf[batch_indices, batch_data['mask'] - 1]
        lambda_t_poi = lambda_t_poi[batch_indices, batch_data['mask'] - 1]
        
        # lambda_t shape: (B, L, C) -> mask -> (N_selected, C)
        indices_expanded = y_POI.unsqueeze(-1).unsqueeze(-1)
        indices_expanded = indices_expanded.expand(-1, lambda_t_poi.shape[1], 1)
        gathered_values = torch.gather(lambda_t_poi, dim=2, index=indices_expanded)
        selected_lambdas = gathered_values.squeeze(-1)
        
        # 1. 获取预测的POI索引 (取最大分数的索引)
        # Shape: (B, L)
        pred_poi_indices = torch.argmax(intensities_weighted, dim=-1)
        # 3. 构建筛选掩码 (Mask)
        # 条件 A: 预测POI == 真值POI
        mask_poi_correct = ((pred_poi_indices == y_POI) | (pred_poi_indices != y_POI)) 
        # 条件 B: 时间误差 <= 2h
        # 注意：确保 dtimes_pred 和 y_time_delta 的单位一致（假设都是小时）
        time_diff = torch.abs(dtimes_pred - y_time_delta)
        mask_time_correct = (time_diff <= 200)
        # 最终 Mask: 两个条件同时满足
        final_mask = mask_poi_correct & mask_time_correct
        # 4. 提取符合条件的数据
        # 使用 nonzero 获取满足条件的 (batch_idx, seq_idx) 坐标
        # indices shape: (N_selected, 2)
        indices = torch.nonzero(final_mask, as_tuple=False)
        
        # 利用掩码提取数据 (Flattened)
        sel_pred_poi = pred_poi_indices[final_mask].cpu().numpy()
        sel_pred_time = dtimes_pred[final_mask].detach().cpu().numpy()
        sel_true_type = y_POI_type[final_mask].detach().cpu().numpy()
        
        sel_true_poi = y_POI[final_mask].cpu().numpy()
        sel_true_time = y_time_delta[final_mask].cpu().numpy()
        
        # 5. 提取高维特征 lambda_t        
        selected_lambdas = selected_lambdas[final_mask].detach().cpu().numpy().tolist()
        selected_pdf = pdf[final_mask].detach().cpu().numpy().tolist()
        
        timestamp = time_seqs[batch_indices, batch_data['mask'] - 1][final_mask].detach().cpu().numpy().tolist()
        
        # 6. 组织成 DataFrame 方便观察
        data = {
            'POI_type': sel_true_type,
            'based_timestamp': timestamp,
            'Pred_POI_ID': sel_pred_poi,
            'True_POI_ID': sel_true_poi,
            'Pred_Time': sel_pred_time,
            'True_Time': sel_true_time,
            'Lambda_t_poi': selected_lambdas
        }
        
        resulting_df = pd.DataFrame(data)
        
        path = '/home/huangyf/TGN/Traj_TPP4NPP/analysis/data_more.csv'
        resulting_df.to_csv(path, mode='a', header=None, index=False)
        '''
        return intensities_weighted_ret, dtimes_pred_ret


    def forward(self, time_seqs, poi_seqs, attention_mask, poi_cat_seq, seq_mask):
    ...
    def compute_states_at_sample_times(self, event_states, sample_dtimes, user_emb, time_seqs, poi_seqs):
        ...
        absolute_sample_times = (time_seqs[..., None] + sample_dtimes * 3600).round().reshape(-1, sample_dtimes.shape[-1]).to(torch.long)
        ...
        return intensity_states, self.layer_user_hidden(user_emb)[:, :, None, :], time_distance_factor, self.time_projector(period_time_embed), self.layer_intensity_hidden(event_states), factor_intensity_base
    ...
    def compute_intensities_at_sample_times(self,
        ...
        enc_out = self.forward(time_seqs=time_seqs, poi_seqs=poi_seqs, attention_mask=attention_mask, poi_cat_seq=poi_cat_seq, seq_mask=seq_mask)    

        # [batch_size, seq_len, num_samples, hidden_size]
        encoder_output, user_int, distance_int, period_int, history_int, base_int = self.compute_states_at_sample_times(enc_out, sample_dtimes, user_emb=self.user_embed_model(user_seq), time_seqs=absolute_time_seqs,
                                                             poi_seqs=poi_seqs)
        ...
        return lambdas, user_int, distance_int, period_int, history_int, base_int
