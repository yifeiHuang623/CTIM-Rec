...
class EventSampler(nn.Module):
    ...
    def draw_next_time_one_step(self, time_seqs, absolute_time_seqs, poi_seqs, user_seqs, poi_cat_seqs, \
        ...
        time_steps = 24
        t_grid = torch.linspace(0, self.dtime_max, time_steps).to(self.device)  
        ...
        return accepted_dtimes.clamp(max=24), weights
...
class FourierPeriodicEncoder(nn.Module):
    ...
    def forward(self, time_dict):
        ...
        # (batch_size, seq_len, fourier_dim)
        fourier_concat = torch.cat(all_fourier_features, dim=-1) 
        ...
        if residual.shape[-1] == projected_features.shape[-1]:
            final_encoding = projected_features + 0.1 * residual  
        else:
