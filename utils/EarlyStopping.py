...
class EarlyStopping(object):
    ...
    def __init__(self, patience: int, save_model_folder: str, save_model_name: str, logger: logging.Logger, model_name: str = None, is_pretrain: bool=False):
        """
        Early stop strategy.
        ...
        self.logger = logger
        if is_pretrain:
            self.save_model_path = os.path.join(save_model_folder, f"{save_model_name}_pretrain.pkl")
        else:
    ...
    def step(self, metrics: list, model: nn.Module):
        ...
        # only average precision and roc
